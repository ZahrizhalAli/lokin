...
from lokin.transports.smallwebrtc.connection import IceServer, SmallWebRTCConnection
    ...
    async def handle_web_request(
        ...
        try:
            ...
            if existing_connection:
                lokin_connection = existing_connection
                logger.info(f"Reusing existing connection for pc_id: {pc_id}")
                await lokin_connection.renegotiate(
                    sdp=request.sdp,
                    type=request.type,
                    restart_pc=request.restart_pc or False,
                )
            else:
                lokin_connection = SmallWebRTCConnection(ice_servers=self._ice_servers)
                await lokin_connection.initialize(sdp=request.sdp, type=request.type)

                @lokin_connection.event_handler("closed")
                ...
                try:
                    await webrtc_connection_callback(lokin_connection)
                    logger.debug(
                        f"webrtc_connection_callback executed successfully for peer: {lokin_connection.pc_id}"
                    )
                except Exception as callback_error:
                    logger.error(
                        f"webrtc_connection_callback failed for peer {lokin_connection.pc_id}: {callback_error}"
                    )

            answer = lokin_connection.get_answer()

            if self._esp32_mode:
                from lokin.runner.utils import smallwebrtc_sdp_munging

                answer["sdp"] = smallwebrtc_sdp_munging(answer["sdp"], self._host)

            self._pcs_map[answer["pc_id"]] = lokin_connection
